...
Quality_hist <- function(infile, pdf_file, width, height){
    ...
    colnames(data) <- c("Chr", "Start", "Chr2", "End", "SVLen", "SVType", "Fst")
    #colnames(data) <- c("Sample", "Total_reads", "Mapped_reads", "Total_bases", "Mapped_bases", "Mapped_base_rate", "Error_rate", "Mapped_read_rate")



    ggplot(data, aes(x=Fst)) +  #Clean_read_length_N50
      geom_histogram(aes(y=..count..), fill="cornflowerblue", color="dodgerblue3",  binwidth=0.001) +
      xlab("Fst") + ylab("Number") +  
      theme_bw()  + 
