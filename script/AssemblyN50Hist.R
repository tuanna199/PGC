...
Quality_hist <- function(infile, pdf_file, width, height){
    ...
    data$ContigN50 <- data$ContigN50 / 1000000

    median_value <- median(data$ContigN50)

    ggplot(data, aes(x=ContigN50)) +  #Clean_read_length_N50
        geom_histogram(aes(y=..count..), fill="cornflowerblue", color="dodgerblue3",  binwidth=0.5) +
      xlab("N50 length (Mb)") + ylab("Sample count") +  
      ...
      geom_vline(xintercept = median_value, color="gray") +
