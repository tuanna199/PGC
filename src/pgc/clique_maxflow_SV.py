...
# usage: python clique_maxflow_SV.py -v {list_vcf} -o {output_file_name}
# parameters
parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter,
                                 description='cliqueSV_maxflow: merge vcf inside a single file \n'
                                             'the sv type: '
                                             '<DEL>, <INS>, <INV>, <DUP>, <INVDUP>, <TRA>, '
                                             '<DUP/INS>, <DEL/INV>, <INV/INVDUP>')
parser.add_argument('-v', '--vcf_list', type=str, default='', help='list of vcf (separated by commas)')
parser.add_argument('-w', '--workers', default=5, type=int, help='number of CPUs to use (default: 5)')
parser.add_argument('-o', '--output', type=str, default='', help='path to the output vcf')
parser.add_argument('-in', '--interval', default=0, type=int,
                    help='segregated all SVs into non-overlapping blocks, interval of block (default: 0)')
parser.add_argument('-af', '--allele_freq', default=0.2, type=float,
                    help='use allele frequency to determine genotype (default: 0.2)')
parser.add_argument('-p', '--overlap_perc', default=0.5, type=float,
                    help='except for <TRA>, overlap percentage of the two SVs (default: 0.5)')
parser.add_argument('-ex', '--extended_dis', default=100, type=int,
                    help='extended distance between the <TRA> start and end upstream and downstream (default: 100)')
...
def main():
    ...
    output_vcf(all_sv_merge, vcf_file_list, contigs, args.output)
    print('-----------------[Output end!]', time.time() - start_time1, 's-----------------')

    log_output(all_sv_form, all_sv_block, all_sv_merge, args.output)
...
def read_vcf(vcf_list, af):
    # all_sv_form[sv_type][chrom][sv_list]
    # all_sv_form['<TRA>'][(chrom1, chrom2)][sv_list]
    contigs = set()
    all_sv_form = {'<DEL>': defaultdict(list),
                   ...
                   '<DUP/INS>': defaultdict(list),
                   '<DEL/INV>': defaultdict(list),
                   '<INV/INVDUP>': defaultdict(list),
                   '<DEL/INVDUP>': defaultdict(list),
                   '<DEL/INV/INVDUP>': defaultdict(list)}

    # read all VCF files
    vcf_file_list = vcf_list.split(',')
        ...
        with open(per_file, 'r') as f:
            for line_raw in f:
                if not line_raw.startswith('#'):
                    ...
                    # other: chrom1
                    if line[4] == '<TRA>':
                        all_sv_form['<TRA>'][(line[0], info_dict['CHR2'])].append(temp_sv)
                    else:
                        all_sv_form[line[4]][line[0]].append(temp_sv)
    ...
    for type_sv in all_sv_form:
        if type_sv != '<TRA>':  # TRA don't have to sort
            for chrom in all_sv_form[type_sv]:
...
def block2clique2merge(all_sv_block, extended_dis, overlap_perc, worker):
    all_sv_merge = {'<DEL>': defaultdict(list),
                    ...
                    '<DEL/INVDUP>': defaultdict(list),
                    '<DEL/INV/INVDUP>': defaultdict(list)}
    print('Start <TRA>: block2clique2merge')
    for chrom12 in all_sv_block['<TRA>']:
        g = nx.Graph()
        for per_sv in all_sv_block['<TRA>'][chrom12][0]:
            g.add_node(per_sv)
            for other_sv in all_sv_block['<TRA>'][chrom12][0]:
                start_per_1, start_per_2 = per_sv.start - extended_dis, per_sv.start + extended_dis
                end_per_1, end_per_2 = per_sv.end - extended_dis, per_sv.end + extended_dis
                start_other_1, start_other_2 = other_sv.start - extended_dis, other_sv.start + extended_dis
                end_other_1, end_other_2 = other_sv.end - extended_dis, other_sv.end + extended_dis
                rate1 = overlap_fun(start_per_1, start_per_2, start_other_1, start_other_2)
                rate2 = overlap_fun(end_per_1, end_per_2, end_other_1, end_other_2)
                if rate1 > 0 and rate2 > 0:
                    g.add_edge(per_sv, other_sv, weight=rate1 + rate2)
        # to find the largest clique
        max_clique_list_tra = []
        temp_nodes_tra = set(g.nodes)
        while temp_nodes_tra:
            temp_graph_tra = nx.Graph.subgraph(g, list(temp_nodes_tra))
            clique_list_tra = [(i, len(i)) for i in nx.algorithms.clique.find_cliques(temp_graph_tra)]
            clique_list_tra = sorted(clique_list_tra, key=lambda x: x[1], reverse=True)
            # take largest clique
            clique_max_tra_num = clique_list_tra[0][1]
            clique_max_tra_list = [i for i in clique_list_tra if i[1] == clique_max_tra_num]
            # sort the elements in the group
            clique_max_tra_list_2 = []
            for per in clique_max_tra_list:
                per = list(per)
                per[0] = sorted(per[0], key=lambda x: x.patient + x.sv_id)
                per = tuple(per)
                clique_max_tra_list_2.append(per)
            # sort group
            clique_max_tra_list = sorted(clique_max_tra_list_2, key=for_list_sort)
            # take the first largest
            clique_max_tra = clique_max_tra_list[0][0]
            max_clique_list_tra.append(tuple(clique_max_tra))
            temp_nodes_tra -= set(clique_max_tra)

        # merge clique
        final_merge_tra = []
        if len(max_clique_list_tra) > 1:
            last_clique_list_tra, stab_clique_list_tra = [], []
            stab_clique_list_tra.extend(max_clique_list_tra)
            while last_clique_list_tra != stab_clique_list_tra:
                last_clique_list_tra = []
                last_clique_list_tra.extend(stab_clique_list_tra)
                stab_clique_list_tra = []
                stab_clique_list_tra.extend(find_stab_clique(last_clique_list_tra, g))
            # a group containing only one element
            clique_list_one = [i[0] for i in stab_clique_list_tra if len(i) == 1]
            clique_list_one = sorted(clique_list_one, key=lambda x: x.patient + x.sv_id)
            # the elements in the group are greater than two
            clique_list_two = [i for i in stab_clique_list_tra if len(i) >= 2]
            # sort the elements in the group
            clique_list_two_2 = []
            for per in clique_list_two:
                per = list(per)
            ...
            for one_sv in clique_list_one:
                ...
                if max_index == 'none':
                    ...
                    new_per_cli.append(one_sv)
                    clique_list_two_2[max_index] = tuple(new_per_cli)
            final_merge_tra.extend(clique_list_two_2)
            all_sv_merge['<TRA>'][chrom12].append(final_merge_tra)
        else:
            final_merge_tra.append(max_clique_list_tra[0])
            all_sv_merge['<TRA>'][chrom12].append(final_merge_tra)
    # multi-processing
    for type_sv in all_sv_block:
        if type_sv != '<TRA>':
                ...
                for per_clique in per_block:
                    ...
                    for per in per_clique_list:
                        if per.patient in vcf_file_list:
                            ...
                            dv = per.dv
                            strands = per.strands
                            start_end = '%s_%s-%s_%s' % (per.chr1, per.start, per.chr2, per.end)
