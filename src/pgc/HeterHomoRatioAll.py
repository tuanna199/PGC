...
def SV_heter_homo_ratio(geno_file, out_file):
    ...
    ChrSampleHeter = collections.Counter()
    ChrSampleHomo = collections.Counter()
    for line in geno_h:
    ...
    for i, s in enumerate(samples):
        if i in ChrSampleHomo:
            SampleHomos = ChrSampleHomo[i]
        else:
            SampleHomos = 0

        if i in ChrSampleHeter:
            SampleHeter = ChrSampleHeter[i]
        else:
            SampleHeter = 0
